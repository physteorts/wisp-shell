...
Singleton {
    ...
    function generateColors(imagePath: string): void {
        ...
        const fullCommand = `export PATH="$HOME/.cargo/bin:$HOME/.local/bin:/usr/local/bin:$PATH"; ` + `cd "${home}/.config/quickshell/wisp-shell/matugen" && ` + `matugen image "${cleanPath}" -c config.toml -m "${mode}" -t scheme-tonal-spot --json hex | ` + `python3 -c "${pyScript}"`;
